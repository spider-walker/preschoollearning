...
export class GamesProvider {
    ...
    getGames(parent_id: number) {
        this.gamesCollection = this.afs.collection<Games>('games', ref => ref.where('parent_id', '==', parent_id));
        this.games = this.gamesCollection.valueChanges();
        return this.games;
    }
    getGamesByUserName(username: string) {
        this.gamesCollection = this.afs.collection<Games>('games', ref => ref.where('username', '==', username));
        this.games = this.gamesCollection.valueChanges();
        return this.games;
    }
    get_children(parent_id: number) {
        this.gamesCollection = this.afs.collection<Games>('dbchildren', ref => ref.where('parent_id', '==', parent_id));
        this.games = this.gamesCollection.valueChanges();
        return this.games;
    }
