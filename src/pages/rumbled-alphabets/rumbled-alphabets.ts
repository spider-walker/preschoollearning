		...
		if (this.rumblesB.length == 0) {
			if (this.arraysEqual(this.rumblesA, this.rumblesC)) {
				...
				if (this.no_of_retries == 3) {
					...
					let toast = this.toastCtrl.create({
						message:
							'Sorry, wrong arrangment! You have lost a point. Game will restart.',
						duration: 3000,
						position: 'middle',
					});
					this.storage.set('pox_rumbled_letters', this.pox_rumbled_letters);
	...
	start_game() {
		...
		setTimeout(function () {
			...
			for (let m = 0; m < showed; m++) {
				let h = my_this.random(
					Math.trunc(my_this.rumblesA.length * 0.3),
					Math.trunc(my_this.rumblesA.length * 0.8)
				);
				hidden.push(h);
			}
